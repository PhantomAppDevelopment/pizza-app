package cz.j4w.map.geo
{
	...
	public class GeoUtils
	{
		 ...
		 */
		public static function distance(lon1:Number, lat1:Number, lon2:Number, lat2:Number):Number
		{
			var v:Number = Math.cos(lat1) * Math.cos(lat2) * Math.cos(lon2 - lon1) + Math.sin(lat1) * Math.sin(lat2);
		 ...
		 */
		public static function distanceDeg(lon1:Number, lat1:Number, lon2:Number, lat2:Number):Number
		{
			return distance(lon1 * DEG_RAD, lat1 * DEG_RAD, lon2 * DEG_RAD, lat2 * DEG_RAD);
		}
		...
		public static function destination(lon:Number, lat:Number, bearing:Number, distance:Number):Point
		{
			...
			var lat2:Number = Math.asin(Math.sin(lat) * Math.cos(a) + Math.cos(lat) * Math.sin(a) * Math.cos(bearing));
			var lon2:Number = lon + Math.atan2(Math.sin(bearing) * Math.sin(a) * Math.cos(lat), Math.cos(a) - Math.sin(lat) * Math.sin(lat2));
		...
		{
			var result:Number = lat2y(lat) - lat2y(Math.asin(Math.sin(lat * DEG_RAD) * COS_1_EARTH_RADIUS + Math.cos(lat * DEG_RAD) * SIN_1_EARTH_RADIUS) * RAD_DEG);
